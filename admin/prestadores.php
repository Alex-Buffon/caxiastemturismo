...
$sql = "SELECT * FROM prestadores ORDER BY tipo ASC, nome ASC";
$res_prestadores = $conn->query($sql);
?>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    ...
    <title>Gerenciar Prestadores de Serviço - Painel GxT</title>
    ...
    <div class="row">
        ...
        <div class="col-md-9 col-lg-10 main-content">
            <div class="d-flex justify-content-between align-items-center mb-4 border-bottom pb-2">
                <h2 class="navbar-brand">Prestadores de Serviço</h2>
                <a href="prestador_form.php" class="btn text-white" style="background-color: #2E4636;">
                ...
                <div class="card-body p-0">
                    <div class="table-responsive">
                        <table class="table table-hover mb-0 align-middle">
                            ...
                            <tbody>
                                <?php if($res_prestadores && $res_prestadores->num_rows > 0): ?>
                                    <?php while($p = $res_prestadores->fetch_assoc()): ?>
                                        <tr>
                                            <td class="ps-4">
                                                <?php if(!empty($p['imagem'])): ?>
                                                    <img src="../img/<?php echo htmlspecialchars($p['imagem']); ?>" width="60" height="40" alt="img">
                                                <?php else: ?>
                                            ...
                                            <td>
                                                <strong class="d-block text-dark"><?php echo htmlspecialchars($p['nome']); ?></strong>
                                            </td>
                                            <td>
                                                <?php 
                                                $tipo = $p['tipo'];
                                                $badge = 'bg-secondary';
                                                $label = $tipo;
                                                if($tipo == 'agencia') { $badge = 'bg-primary'; $label = 'Agência'; }
                                                elseif($tipo == 'agente') { $badge = 'bg-success'; $label = 'Agente'; }
                                                elseif($tipo == 'transportador') { $badge = 'bg-info text-dark'; $label = 'Transportador'; }
                                                ?>
                                            ...
                                            <td class="text-end pe-4">
                                                <a href="prestador_form.php?id=<?php echo $p['id']; ?>" class="btn btn-sm btn-outline-primary" title="Editar">
                                                    <i class="bi bi-pencil"></i>
                                                </a>
                                                <a href="prestador_delete.php?id=<?php echo $p['id']; ?>" class="btn btn-sm btn-outline-danger ms-1" onclick="return confirm('Tem certeza que deseja apagar o prestador <?php echo htmlspecialchars(addslashes($p['nome'])); ?>?');" title="Excluir">
                                ...
                                <?php else: ?>
                                    <tr>
                                        <td colspan="5" class="text-center py-5 text-muted">Ainda não há nenhum prestador cadastrado. <br>Eles aparecerão nas páginas de Agências, Agentes e Transportadores!</td>
                                    </tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
